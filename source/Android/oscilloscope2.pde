...
void setup() {
  background(0);
  frameRate(50);
  port = new Serial(this, Serial.list()[0], 115200); //port = new Serial(this, Serial.list()[0], serialBaudRate);
  //port.setDTR(true);
}
...
void draw() {
  ...
  windowHeight = height-200; //-100
    ...
    //text("List "+Serial.list()[0], 400, 400);
    ...
    if (hold) {
      ...
      while (port.available ()>=3) { //                                                             // "Дальше": работа в обычном режиме
        writeIndex++;  
